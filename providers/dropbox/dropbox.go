...
const (
	dropboxDefaultScope    string = ""
	dropboxName            string = "dropbox"
	dropboxDisplayName     string = "Dropbox"
	dropboxAuthURL         string = "https://www.dropbox.com/1/oauth2/authorize"
	dropboxTokenURL        string = "https://www.dropbox.com/1/oauth2/token"
	dropboxEndpointProfile string = "https://api.dropbox.com/1/account/info"
)

// DropboxProvider implements the Provider interface and provides Dropbox
...
func New(clientId, clientSecret, redirectUrl string) *DropboxProvider {

	p := new(DropboxProvider)
	p.config = &common.Config{Map: objx.MSI(
		oauth2.OAuth2KeyAuthURL, dropboxAuthURL,
		oauth2.OAuth2KeyTokenURL, dropboxTokenURL,
		oauth2.OAuth2KeyClientID, clientId,
		oauth2.OAuth2KeySecret, clientSecret,
		oauth2.OAuth2KeyRedirectUrl, redirectUrl,
		oauth2.OAuth2KeyScope, dropboxDefaultScope,
...
func (provider *DropboxProvider) TripperFactory() common.TripperFactory {
...
func (provider *DropboxProvider) PublicData(options map[string]interface{}) (interface{}, error) {
	return gomniauth.ProviderPublicData(provider, options)
}

// Name is the unique name for this provider.
func (provider *DropboxProvider) Name() string {
	return dropboxName
}

// DisplayName is the human readable name for this provider.
func (provider *DropboxProvider) DisplayName() string {
...
func (provider *DropboxProvider) GetBeginAuthURL(state *common.State, options objx.Map) (string, error) {
	if options != nil {
		scope := oauth2.MergeScopes(options.Get(oauth2.OAuth2KeyScope).Str(), dropboxDefaultScope)
...
func (provider *DropboxProvider) Get(creds *common.Credentials, endpoint string) (objx.Map, error) {
...
func (provider *DropboxProvider) GetUser(creds *common.Credentials) (common.User, error) {

	profileData, err := provider.Get(creds, dropboxEndpointProfile)
...
func (provider *DropboxProvider) CompleteAuth(data objx.Map) (*common.Credentials, error) {
...
// protected Dropbox resources
func (provider *DropboxProvider) GetClient(creds *common.Credentials) (*http.Client, error) {
